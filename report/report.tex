...
\begin{document}
  ...
  For a given current density $\vec{j}(t): \Omega \to \R^3$ (\footnote{
    The current density satisfies $\vec{j} = nq\vec{v}$, describing the flow of $n$ charges $q$ with velocity $\vec{v}$, it is therefore the amount of current $I$ flowing through an infinitesimal area $A$, motivating its notation as a \textit{density}.
  ...
  \end{align*}
  according to the explicit representation of the cross-product in three spatial dimensions, most notably allows us to express $B_1$, $B_2$ and $B_3$ as convolution integrals
  ...
  $$\mathcal{F}[f * g](\vec{k}) = \mathcal{F}[f](\vec{k}) \cdot \mathcal{F}[g](\vec{k}) \quad\text{where}\quad \mathcal{F}[f](\vec{k}) := \int f(\vec{x}) \e^{-\i \vec{k}^T \vec{x}}\idd\vec{x}\,,$$
...
function calculate_magnetic_field(cdp::CurrentDensityPhantom)::VectorField
  ...
  As a regulariser $R(\sigma)$ for the conductivity $\sigma$, we used the \gls{tv}, 1-norm of the gradient, of $\sigma$.
